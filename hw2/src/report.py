...
def memory_section(report: dict) -> list[str]:
    ...
    lines += [
        ...
        f"инференса. Full fine-tune дороже LoRA в {modes['full_ft']['peak_mb'] / modes['lora']['peak_mb']:.2f} раза.",
        "",
    ]
    if base["device"].startswith(("mps", "cuda")):
        lines += [
            f"В колонке «Пик RSS» разброс между режимами — "
            f"{rss_spread:.0f} МБ на все три — и это при том, что full fine-tune "
            f"обязан добавить к весам ещё {3 * weights_mb:.0f} МБ.",
            f"`{base['rss_source']}` описывает резидентную память процесса. Она не является",
...
def write_report(report: dict, params: dict) -> None:
    """Обновить график и отчёт, сохранив ручные пояснения после маркера."""
    plot_activations(report["activations"], params["hooks"]["plot"])
    path = Path(params["report"]["markdown"])
    path.parent.mkdir(exist_ok=True)
    marker = "<!-- manual-notes -->"
    previous = path.read_text(encoding="utf-8") if path.exists() else ""
    _, separator, notes = previous.partition(marker)
    text = markdown_report(report, params)
    if separator:
        text += "\n" + marker + notes
