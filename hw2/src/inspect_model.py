...
def activation_norms(tokenizer, model, params: dict) -> dict:
    ...
    try:
        ...
        for module, state in training.items():
...
class PeakMemory:
    """CUDA high-water mark; MPS — максимум опроса драйвера и границ этапов."""
    ...
    for mode in MODES:
        ...
        for _ in range(repeats):
            process = subprocess.run(
                ...
                env={**os.environ, "HW2_PROBE_PARAMS_STDIN": "1"},
            )
...
def main() -> None:
    ...
    params = (json.load(sys.stdin) if os.environ.get("HW2_PROBE_PARAMS_STDIN") == "1"
              else load_params())
    set_seed(params["generate"]["seed"])
