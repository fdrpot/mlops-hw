...
if uv run python - > "$log" 2>&1 <<'PY'
from src.config import load_params
from src.inspect_model import activation_norms, forward_hooks, resolve_device
from src.model import load_model
...
assert [m.training for m in model.modules()] == states, "режимы модулей не восстановлены"
after_first = hooks_left()
second = activation_norms(tokenizer, model, params)
assert first == second, "повторный прогон изменил нормы"
assert [m.training for m in model.modules()] == states
after_second = hooks_left()

try:
    with forward_hooks({"model": model}):
        raise RuntimeError("проверка очистки при исключении")
except RuntimeError:
    pass
assert hooks_left() == 0, "хуки остались после исключения"
